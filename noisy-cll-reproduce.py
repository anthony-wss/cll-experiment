...
def get_noisy_cifar10(noise_set, eta):
    transform = transforms.Compose(
        [
            transforms.ToTensor(),
            transforms.Normalize(
    ...
    if noise_set == "sym":
        U = torch.full([num_classes, num_classes], eta / (num_classes - 1))
        for i in range(num_classes):
            U[i][i] = 1-eta
    elif noise_set == "asym":
        pass
...
def train(args):
    ...
    elif dataset_name == "sym-noise-cifar10":
        trainset, validset, testset, ord_trainset, ord_validset = get_noisy_cifar10("sym", args.eta)
    else:
        raise NotImplementedError
    ...
    if algo in ["fwd-u", "ure-ga-u"]:
        ...
        for i in range(num_classes):
            Q[i][i] = 0
    elif algo in ["fwd-r", "ure-ga-r", "rob-mae"]:
        Q = dataset_T
        ...
        for epoch in tepoch:
            ...
            for inputs, labels in trainloader:
                ...
                if algo == "scl-exp":
                    ...
                    loss = noisy_ga_loss(outputs, labels, class_prior, Q)
                    if torch.min(loss) > 0:
                        loss = loss.sum()
                        loss.backward()
                    else:
